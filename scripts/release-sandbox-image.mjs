...
if (publish) {
  ...
  process.stdout.write(`${JSON.stringify({ image: `${publish.split("@")[0]}@${remoteDigest}`, tag: publish,
    source, version: pkg.version, runnerSha256: digest(runner), base }, null, 2)}\n`);
} else if (build) {
  run("docker", ["build", "--platform", "linux/amd64", "--tag", build, "sandbox"]);
  run("docker", ["run", "--rm", "--platform", "linux/amd64", build,
    "/usr/local/bin/constal-workspace-runner", "probe"]);
  process.stdout.write(`${JSON.stringify({ image: build, source, version: pkg.version,
    runnerSha256: digest(runner), base, verified: true }, null, 2)}\n`);
} else {
