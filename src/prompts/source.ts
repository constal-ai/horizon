...
Return ready only when one repository is supported by authenticated GitHub evidence. If multiple repositories remain genuinely plausible, ask one concise question that distinguishes them. If GitHub access or repository evidence is unavailable, report the exact blocker.`,
  ...
  output: `Return exactly one JSON object:
{
  ...
  "blockedReason":"specific blocker or null"
}

ready requires source. needs-input requires question. blocked requires blockedReason.`,
});
