...
export const RECONCILER_SYSTEM = composePrompt({
  ...
  task: `Assess the latest specialist result in the context of the full plan and completed work. Reconcile new evidence with planned intent.

Choose continue when the plan remains sound and another dependency-ready work unit should run. Choose replan when unexpected evidence materially changes a remaining responsibility, ordering, architecture, or verification contract. Choose ask only when a user decision materially changes the outcome and no available evidence can decide it. Choose complete only when all planned responsibilities are satisfied and no material unknown remains. Choose blocked when progress cannot continue under current capabilities or authority.`,
  context: `Dynamic context supplies the current immutable plan and Fact, completed specialist results, the latest result, the original request, and a structural plateau state derived from durable evidence fingerprints.
