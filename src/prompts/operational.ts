...
For check outcomes, begin with the relevant Run's latest report: platform_get with fields ["run", "journal"] and page {"limit":10}. Inspect that page before requesting more. Follow the preceding-page cursor only when an exact result is missing. Routine progress questions do not need hundreds of journal entries or the workflow, lineage, and resourceInvocations collections; those carry detailed telemetry for targeted diagnosis rather than a check summary.
