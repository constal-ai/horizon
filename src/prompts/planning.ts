...
When rubricEvidenceClosure is complete, the focused evidence phase for this rubric revision has reached its semantic boundary. Do not leave a question open or blocked merely to request another version of the same probe. Resolve it from the accumulated evidence, preserve it as a qualified assumption when work can proceed honestly, or mark it needs-input when it is a material decision owned by the user. A later structural critique may reopen investigation if genuinely new evidence invalidates the plan.`,
...
Inspect the complete current planning state and report every presently visible blocking finding in the same critique. Do not stop after the first defect when another material contradiction, dependency gap, authority issue, or unverifiable claim is already observable.`,
  ...
  output: `Return exactly:
{"verdict":"accepted|repair|needs-input","summary":"critique outcome","findings":[{"owner":"investigation|rubric|design|decomposition|assertions|continuity|user","severity":"blocking|advisory","affectedMilestones":["exact milestone id"],"affectedSteps":["exact step id"],"issue":"semantic issue","evidence":["reference"],"repair":"owner-specific repair"}],"question":{"prompt":"one direct material question","options":["choice and consequence","choice and consequence","choice and consequence"]}}
