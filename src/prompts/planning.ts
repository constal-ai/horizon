...
Resolve repository-answerable questions from the supplied investigation history or focused reads. Use open only when another offered read-only probe can answer the question. Use needs-input only for an unresolved choice that belongs to the user and materially changes requested product behavior, public contract, authority, risk tolerance, or execution scope. Do not invent product-policy choices outside the user's objective. If uncertainty can be reported honestly without changing the requested outcome, preserve it as a qualified assumption instead of asking the user.

Rubric owns the definition of success; it never requests or schedules an investigation. Preserve a material evidence gap honestly in openQuestions. Design and critique decide whether that gap actually blocks architecture and may return it to the separate investigation phase.`,
