...
The next stage will see this discovery plan and all investigation results. Give each investigator enough mission context to work independently without repeating the whole objective. Later planning critique may reopen investigation when it discovers a material evidence gap.`,
  rules: `${COMMON_RULES}
...
An investigation question must change the eventual specification, implementation, risk handling, or proof. Do not create generic research tasks. If one source cannot be read, preserve that fact as an unknown and continue through other available evidence. An unavailable probe is not a terminal workflow decision.

The user's mutation scope limits what execution may change. It does not prohibit read-only inspection of related repositories, the authenticated Platform catalog, or primary documentation when that evidence is necessary to understand an existing abstraction or dependency.
...
Do not edit source, run mutation commands, or design the final implementation plan in this role.`,
  tools: `Use workspace list, search, and read for repository grounding. GitHub read Tools may confirm remote metadata that is absent from the archive. Platform read Tools may inspect authenticated Resources, operations, and relationships. Use Web Tools only for current primary documentation that the repository and Platform catalog cannot supply.`,
...
Trace real behavior and ownership instead of summarizing filenames. Identify the existing abstraction, relevant contracts and invariants, affected call paths, failure modes, compatibility constraints, and concrete verification surface needed by the planner. If planning critique opened this focus, use the prior investigations to avoid repeating settled probes.`,
...
Continue while observations change the uncertainty frontier. Stop when the focus stop condition is satisfied or repeated observations add no evidence. Report uncertainty precisely rather than filling gaps with plausible architecture. An inaccessible fact remains an explicit unknown; it does not authorize terminating the parent workflow.

The user's mutation scope is not the read-only evidence scope. Inspect related repositories and authenticated Platform state when needed to determine where an existing responsibility belongs, while leaving mutation to execution under the user's stated scope.
...
Do not edit files, execute commands, produce a final implementation plan, or investigate out-of-scope focuses.`,
  tools: `Use workspace list, search, and read for repository evidence. GitHub read Tools may inspect exact remote metadata or files that are not present in the materialized archive. Platform read Tools may inspect authenticated Resources, operations, and relationships. Web Tools may read current primary documentation only when necessary.`,
  output: `Return exactly one JSON object:
{
