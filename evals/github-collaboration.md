...
The finalizer adjustment (`5ea98af`) passed all 206 tests and was deployed as `d00981f5-804c-48c2-8613-25e322ce2552`. Existing attempts remain on their admitted revisions.

- Issue #2 executor `9943ab38-ba07-813b-8226-288ca7ebfbec` applied the source and regression-test changes. Its first full-memory checkpoint retained those edits, final status, and the actual failed checks (`vitest` and `tsc` were not yet installed). It then used the existing command Tool to install dependencies, rather than requiring a platform change. Check completion remains unverified.
- Issue #3 replacement discovery recognized the preserved guide and README edits and selected one focused investigation of their correctness, rather than recreating the work. Its plan is still in progress.
- Issue #4's [replacement proposal](https://github.com/constal-ai/const-alpha/issues/4#issuecomment-5554808532) was read in full and checked against `onMessage` and `executeHandoffs`: discovery alone catches `GateExhausted`; specialist failures are outside that catch. The plan received [approval](https://github.com/constal-ai/const-alpha/issues/4#issuecomment-5554815529). Implementation and PR verification remain pending.

The live observation helper now reports a completed root's actual application result, not just the runtime's `complete` status, and selects specialist results only from the current root's descendants. This prevents historical successes or application-level blocked results from being mistaken for current acceptance evidence. No PR or passing end-to-end result is claimed yet.

### Current implementation and conversation checks

Issue #3's [full replacement proposal](https://github.com/constal-ai/const-alpha/issues/3#issuecomment-5554823171) was reviewed and [approved](https://github.com/constal-ai/const-alpha/issues/3#issuecomment-5554835052). Its executor completed and independent verifier `bf27059b-d7ba-8b0c-96e1-cc9d839be303` started. Issue #4's guide executor also completed; verifier `35c1cd28-7538-8d09-9e8a-e28867d296af` is active.

Issue #2's executor installed dependencies successfully, corrected two bad expectations in its newly authored tests without changing the agreed behavior, and passed focused tests (17), typecheck, build, the full suite (351 tests), and `npm run check`. Native command receipts were read directly to confirm exit code zero and the actual suite output. Verifier `47821c06-e5d6-874b-bc00-b22f73b65b22` then started. These results establish executor progress, not yet independent acceptance or a PR.

A [status question during execution](https://github.com/constal-ai/const-alpha/issues/2#issuecomment-5554843459) exposed a separate supervisor defect. Its [reply](https://github.com/constal-ai/const-alpha/issues/2#issuecomment-5554849460) incorrectly said checks were still ahead, although their receipts already showed success. Conversational Run `351ac5ba-0661-486f-8a77-61b81c245921` made no on-demand journal read and selected `guide-work`, turning the informational reassurance to keep working into steering. The active work was not restarted or its history edited to hide this failure.

The correction clarifies that scheduler metadata does not establish check outcomes, directs progress questions to existing journal/result reads, distinguishes executor reports from independent verification, and treats informational “keep working” as `respond` unless the user changes the work. A controller regression test carries a recorded journal report through the existing Tool loop and verifies that a response applies no control operation. The test does not claim to validate the model's semantic choice; a fresh live status question is required. Typecheck and all 207 tests pass. No new platform endpoint, state projection, prose validator, or parallel routing mechanism was introduced.

### Recovery context and artificial capability denial

The first reporting correction was deployed as `238fdafe-09fb-4f96-83ec-99ee250363b3`. The [repeated question](https://github.com/constal-ai/const-alpha/issues/2#issuecomment-5554897568) was read-only and did use two journal reads, but its [answer](https://github.com/constal-ai/const-alpha/issues/2#issuecomment-5554904996) was still wrong: Run `97cda8b0-e680-4a1e-a322-c75ce402f112` inspected the newer planner and design rather than the earlier executor. The Run index omits specialist roles. The existing Session view exposes them, so the supervisor now receives its exact `workSessionRef` and instructions for an on-demand `state.runs` read through the same `platform_get` Tool. No platform projection was added.

Independent verifier `47821c06-e5d6-874b-bc00-b22f73b65b22` confirmed issue #2's behavior and most checks but omitted its own `npm run check`. The reconciler correctly selected `reverify`. Source inspection found that the next verifier was given neither the previous verifier report nor the original execution receipts; user-driven replanning also omitted the latest partial execution evidence. Commit `c95844d` preserves the existing attempt record, receipts, and current workspace state in verification, and uses the latest execution evidence across user-driven planning revisions. A regression test first failed on the missing handoff and then passed. It also covers new guidance arriving during forward repair without losing the partial attempt.

The same older-loop problem recurred in issue #3's verifiers: they reported missing ordinary file reads despite those capabilities being available. The shared ReAct controller was silently disabling Tools after repeated observations. The correction removes this muting and its staged Tool subsets. Repetition remains visible as feedback alongside retained working memory; declared role Tools remain available until the existing explicit turn budget is exhausted. Policy, role scope, and budgets are unchanged. Tests cover recovering from repeated diffs by reading the missing file and running the remaining check. Typecheck and all 209 tests pass.

The native Session-reference and recovery-context changes were deployed together as `3a6debf8-f629-4077-b9c1-d9d41e6ad95d` before the Tool-muting removal. The [next status question](https://github.com/constal-ai/const-alpha/issues/2#issuecomment-5554989518) remains to be assessed. The first misclassified status message had already caused an unnecessary replan in the older pinned work Run. Its expanded [proposal](https://github.com/constal-ai/const-alpha/issues/2#issuecomment-5554961478) was [rejected](https://github.com/constal-ai/const-alpha/issues/2#issuecomment-5554973664); the corrected [verification-only proposal](https://github.com/constal-ai/const-alpha/issues/2#issuecomment-5555006014) preserves the implemented source and focused tests.

No case has yet produced a verified PR. Keep the live attempts and their actual application outcomes distinct from unit-test results.

Tool-muting removal `b4afc3d` was deployed as `301925ba-24a1-4f95-99e3-c7d4f2905c39`; the 209-test suite passed. The verification-only issue #2 plan received [approval](https://github.com/constal-ai/const-alpha/issues/2#issuecomment-5555027875).

Issue #4 root `97c217c4-92c7-441f-8c6f-0c6952f1f457` completed both work units and their independent verification. It then returned an application-level blocked result when publication's first `git.ref.get` failed. The durable publication-failure Fact is `57397f04ebbd5249110e6915cec4d730445fb506db21a20900622feb38f9152f`, artifact record `09b06e58163abc2403cbe52d0215b518ced0f6b64409df8d61cc3fcdf6c252a1`, plan Fact `0a11bacffeedfe25e3cd88c2fdb6101cd1bdbc4d4a301aa8518e87ee33df163c`. Its verified 579,452-byte workspace archive remains at `9eb3ec6d64ff01911c16c589d0d707e3cf1598e6aee03100045052ba636fe04c`. No PR was manually created.

Source inspection locates the publication failure at the new-branch existence probe, before artifact import or blob creation. The GitHub Tool correctly treats an error with numeric `status: 404` as absence, but DriverHost, Resource invocation error normalization, and runtime error reconstruction preserve only selected metadata and omit that status. The journal consequently records only `code: ApiError` and its message. The required fix is generic HTTP-status preservation through the error transport, not GitHub-specific logic or parsing the prose message. No core error-transport change has been made yet.

The status question sent against the Session-reference candidate [failed](https://github.com/constal-ai/const-alpha/issues/2#issuecomment-5554996784) with `409 Driver governance planner failed`; the Session lookup still needs its exact failure diagnosed. The Session state itself has task labels and is modest in size (about 143 KB for the 132-Run work session), so an oversized model context has not been established as the cause.

### Shared publication transport correction

All three current work roots reached verified work and then failed the same new-branch existence probe. Core commit `c8b9e429` introduces one private error-metadata helper shared by DriverHost, invocation normalization, and Runner reconstruction. It preserves numeric HTTP status while retaining existing provenance and redaction boundaries; it does not parse error prose or add a GitHub-specific rule. Four new boundary/replay regressions failed before the fix and passed afterward. Metadata, DriverHost, and Runner suites pass; the combined broader run is 173/174 with one unchanged owner-route source-string assertion failure also present on the prior `main`. Platform, Service Driver, Runtime, and builder typechecks pass.

The fix is deployed to platform `59e0e963-bdde-4d97-841e-8aec298e2eb1`, Service Driver Worker version `bb102a79-6db0-4191-8d50-d32260edb99d`, and builder `bb882f06-e39f-45ee-9e4a-ddeefe1ad412`. The builder's active container application is version 104, image `sha256:60ce3210328a5a9239893549ab77496900dd245a70da338da914b0dbc0aa92e7`, with no active rollout. Horizon still needs rebuilding with this runtime before restarting publication attempts.

The Session-reference status lookup was valid: conversational Run `de06409e-90b4-4b73-bed4-961bbef72f14` read `state.runs` and located the correct completed executor and verifier. It then requested two 200-entry traces including workflow, lineage, and invocation telemetry (473,868 and 218,331 bytes), in addition to the 139,598-byte Session response and original context. The next model-governance preparation failed. Retrieval guidance now starts with only Run and journal, ten latest entries, expanding on demand; no stored result is truncated and no model ceiling is raised. All 209 Horizon tests pass.
